        ...
        <div ref={containerRef} className="h-full flex flex-col bg-white">
            ...
            <div className="flex items-center bg-[var(--background)] border-b border-[var(--border-color)] select-none shrink-0">
                <div className="flex items-center px-1">
                    <button onClick={() => setShowSidebar(!showSidebar)} className="p-2 text-[var(--foreground)] opacity-30 hover:opacity-50 transition-opacity" title="Toggle snippets">
                ...
                <div className="flex flex-1 overflow-auto">
                    {tabs.map(tab => (
                        ...
                        >
                            ...
                            {tab.name}
                            {tabs.length > 1 && (
                                <button onClick={(e) => { e.stopPropagation(); removeTab(tab.id); }} className="ml-1 opacity-30 hover:opacity-60 transition-opacity">
                                    <X size={10} />
                                </button>
                            )}
                        </div>
                    ))}
                    <button onClick={addTab} className="px-2 py-1.5 opacity-30 hover:opacity-50 transition-opacity" title="New tab">
                        <Plus size={14} />
                    </button>
                </div>
                <div className="flex items-center gap-0.5 px-2">
                    ...
                    <div className="relative">
                        <button onClick={() => setShowFontSizeMenu(!showFontSizeMenu)} className="p-1.5 text-[var(--foreground)] opacity-30 hover:opacity-50 transition-opacity" title="Font size">
                            <Settings2 size={13} />
                        </button>
                        {showFontSizeMenu && (
                    ...
                    <button onClick={() => copyClip(activeTab.code, 'code')} className="p-1.5 text-[var(--foreground)] opacity-30 hover:opacity-50 transition-opacity" title="Copy code">
                        {copiedId === 'code' ? <Check size={13} /> : <Copy size={13} />}
                    </button>
                    <button onClick={downloadCode} className="p-1.5 text-[var(--foreground)] opacity-30 hover:opacity-50 transition-opacity" title="Download">
                        <Download size={13} />
                    </button>
                    <button onClick={shareCode} className="p-1.5 text-[var(--foreground)] opacity-30 hover:opacity-50 transition-opacity" title="Share">
                        <Share2 size={13} />
                    </button>
                    <button onClick={() => { activeTab.code.split('\n').map(l => l.trimEnd()).join('\n').replace(/\n{3,}/g, '\n\n'); }} className="p-1.5 text-[var(--foreground)] opacity-30 hover:opacity-50 transition-opacity" title="Format">
                        <Code2 size={13} />
                    </button>
                    <div className="w-px h-3 bg-[var(--border-color)] mx-1" />
            ...
            <div className="flex-1 flex min-h-0">
                ...
                {showSidebar && (
                    <div className="w-48 shrink-0 bg-[var(--background)] border-r border-[var(--border-color)] flex flex-col">
                        <div className="px-3 py-2 text-[10px] font-mono font-bold text-[var(--foreground)] opacity-30 uppercase tracking-wider border-b border-[var(--border-color)]">
                            Snippets
                        </div>
                        ...
                        <div style={{ height: `${(1 - splitRatio) * 100}%` }} className="min-h-0 flex flex-col bg-[var(--background)]">
                            <div className="flex items-center justify-between px-3 py-1 bg-[var(--background)] border-y border-[var(--border-color)] shrink-0">
                                <div className="flex items-center gap-3">
                                    ...
                                    {outputs.length > 0 && (
                                        <span className="text-[10px] font-mono text-[var(--foreground)] opacity-30 bg-black/5 px-1.5 py-0.5 rounded">
                                            {outputs.length}
                                        </span>
                                    )}
                                    {elapsedTime !== null && (
                                    ...
                                    {outputs.length > 0 && (
                                        <>
                                            <button onClick={() => { const t = outputs.filter(o => o.type !== 'info').map(o => o.content).join('\n'); copyClip(t, 'out'); }}
                                                className="p-1 text-[var(--foreground)] opacity-30 hover:opacity-40 transition-opacity" title="Copy output">
                                                {copiedId === 'out' ? <Check size={11} /> : <Copy size={11} />}
                                            </button>
                                            <button onClick={clearOut} className="p-1 text-[var(--foreground)] opacity-30 hover:opacity-40 transition-opacity" title="Clear">
                                                <Trash2 size={11} />
                                            </button>
                                        </>
                                    )}
                                    <button onClick={() => setShowHistory(!showHistory)} className={`p-1 transition-opacity ${showHistory ? 'opacity-50' : 'opacity-30 hover:opacity-40'}`} title="History">
                                        <History size={11} />
                                    </button>
                                </div>
                            </div>
                                ...
                                <div className="border-b border-[var(--border-color)] max-h-[140px] overflow-y-auto bg-[var(--background)] shrink-0">
                                    ...
                                    {history.map((e, i) => (
                                        <div key={i} onClick={() => loadHist(e)}
                                            ...
                                            <span className="text-[10px] font-mono text-[var(--foreground)] opacity-30 shrink-0 ml-2">{new Date(e.timestamp).toLocaleTimeString()}</span>
                                        </div>
                                    ))}
                                </div>
                            )}
                            ...
                            <div ref={outputRef} className="flex-1 overflow-y-auto min-h-0">
                                <pre className="p-3 m-0 font-mono text-[12px] leading-[1.6] whitespace-pre-wrap">
                                    {outputs.length === 0 ? (
                                        <span className="text-[var(--foreground)] opacity-30 italic">Press ⌘+Enter or click Run to execute...</span>
                                    ) : outputs.map((e, i) => (
                                        <div key={i} className={
                                            e.type === 'error' ? 'text-red-600' :
                                            e.type === 'info' ? 'text-[var(--foreground)] opacity-30 italic' :
