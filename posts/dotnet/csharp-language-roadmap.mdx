...
性能敏感场景还需要了解 `Span<T>` 和 `Memory<T>`：`Span<T>` 是 `ref struct`，在栈上操作连续内存，避免堆分配；`Memory<T>` 是可以存储在堆上的版本，适合异步场景。日常业务代码很少直接使用，但在处理大量数据、序列化、网络缓冲区等场景中很重要。
...
异步编程的延伸能力包括 `Channel<T>`（异步生产者-消费者管道）和 `IAsyncEnumerable<T>`（异步流），它们在消息处理、实时数据推送等场景中很常用。详情可参考 [C# async/await 异步编程从原理到实践](/posts/dotnet/csharp-async-await)。
...
graph TD
  ...
  J --> N
  L --> N
  J --> O[Channel 与 IAsyncEnumerable]
  D --> P[Span 与 Memory]
```
...
- 现代语法让模型表达更简洁、更安全
- `Channel<T>` 和 `IAsyncEnumerable<T>` 是异步编程的延伸，构建异步数据流和管道
- `Span<T>` 和 `Memory<T>` 是类型系统的延伸，在性能敏感代码中避免堆分配和拷贝
