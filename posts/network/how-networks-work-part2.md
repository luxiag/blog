...
tags:
  - 网络是怎样连接的
date: 2021-11-15
excerpt: 深入协议栈内部，从创建套接字、连接服务器、TCP收发数据、断开连接，到IP与以太网的包收发操作，完整理解数据如何在电信号层面传输。
nextPost: network/how-networks-work-part3
---

上一篇我们了解了浏览器如何生成HTTP请求消息并通过DNS查询获取服务器IP地址。接下来，数据将被交给操作系统内部的协议栈来处理——协议栈是如何管理套接字、建立连接、收发数据，最终将数字信息转换为电信号在网线上传输的？本文将沿着这条链路逐一展开：

1. **协议栈的内部结构** → 了解TCP/IP模块如何分工
2. **创建套接字** → 通信的起点
3. **连接服务器** → 三次握手与控制信息
4. **收发数据（TCP）** → 可靠传输的核心机制
5. **断开连接** → 四次挥手与套接字删除
6. **IP与以太网的包收发** → 从逻辑地址到物理信号

## 创建套接字

应用程序（如浏览器）在收发数据之前，首先需要创建一个套接字。套接字是协议栈进行通信操作的基础，理解它的内部结构有助于理解后续的连接、收发和断开过程。
...
创建套接字之后，应用程序（浏览器）就会调用connect，随后协议栈会将本地的套接字与服务器的套接字进行连接。连接的本质是双方交换控制信息，使得彼此的套接字进入可以收发数据的状态。
...
连接建立之后，就进入了数据收发阶段。TCP模块需要将应用程序交付的数据可靠地发送给对方，并确保数据不丢失、不乱序。这涉及数据拆分、ACK确认、窗口控制等关键机制。
...
协议栈会检查收到的数据块和TCP 头部的内容，判断是否有数据丢失，如果没有问题则返回ACK号
...
数据收发完毕后，通信双方需要断开连接并释放套接字占用的资源。断开连接的过程同样是基于TCP头部中的控制位来协调的，与连接时一样需要双方交换确认信息。
...
前面我们一直在讨论TCP层面的连接、收发和断开。但TCP模块生成的数据块并不能直接在网线上传输——它需要委托IP模块将数据封装成网络包，再由以太网将包转换成电信号发送出去。这一层是从逻辑地址到物理传输的桥梁。
...
回顾整篇文章，数据从应用程序到网线的传输经历了以下完整流程：

1. **创建套接字**：协议栈分配内存空间并写入初始状态，为后续通信做准备
2. **连接服务器**：客户端与服务端通过TCP头部交换控制信息（SYN/ACK），建立连接
3. **收发数据（TCP）**：TCP模块将数据拆分为合适大小的包，通过ACK号确认送达，用窗口机制控制发送节奏，合并ACK与窗口更新以提升效率
4. **断开连接并删除套接字**：双方通过FIN/ACK完成四次挥手，释放套接字资源
5. **IP与以太网的包收发**：IP模块添加IP头部和MAC头部，通过ARP查询MAC地址，网卡将数字信息转换为电信号加上报头和帧校验序列，最终通过PHY模块发送到网线

从套接字的创建到电信号的产生，协议栈的每一层都在为数据的可靠传输和正确路由服务。下一篇我们将继续追踪网络包从集线器出发后的旅程。
