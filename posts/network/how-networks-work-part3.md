...
tags:
  - 网络是怎样连接的
date: 2021-11-21
excerpt: 信号从计算机流出后，如何在网线和集线器中传输？交换机如何根据 MAC 地址表转发包？路由器如何基于 IP 进行包转发？地址转换和包过滤又是如何工作的？本文沿着信号从网卡出发的路径，逐步讲解集线器、交换机、路由器的核心工作机制。
nextPost: network/how-networks-work-part4
---

上一篇我们了解了协议栈如何将数据封装成网络包并交给网卡发送。当信号从网卡流出后，它要经过网线、集线器、交换机、路由器等一系列网络设备，才能最终到达目的地。本文将沿着信号的传输路径，依次讲解四个阶段：

1. **信号在网线和集线器中传输** —— 信号如何在物理介质上前进，以及如何应对衰减和噪声
2. **交换机的包转发操作** —— 交换机如何根据 MAC 地址表将包精准送达
3. **路由器的包转发操作** —— 路由器如何基于 IP 地址和路由表进行跨网络转发
4. **路由器的附加功能** —— 地址转换（NAT）和包过滤如何工作
...
网卡中的 PHY(MAU)模块负责将包转换成电信号，信号通过 RJ-45 接口进入双绞线以太网信号的本质是正负变化的电压
...
> 噪声是如何产生的
> 产生噪声的原因是网线周围的电磁波，当电磁波接触到金属等导体时，在其中就会产生电流。因此，如果网线周围存在电磁波，就会在网线中产生和原本的信号不同的电流。信号和噪声的电流就会混杂在一起，导致信号的波形发生失真，这就是噪声的影响

将信号线缠绕在一起，信号线就变成了螺旋形，其中两根信号线中产生的噪声电流方向就会相反，从而使得噪声电流相互抵消，噪声就得到了抑制

![](./images/129185334.png)

![](./images/129185527.png)
...
![](./images/129193904.png)

**集线器只是原封不动地将信号广播出去，所以即便信号受到噪声的干扰发生了失真，也会原样发送到目的地。接收信号的设备，也就是交换机、路由器、服务器等，会在将信号转换成数字信息后通过 FCS 校验发现错误，并将出错的包丢弃。**

了解了信号在物理层面的传输方式后，接下来我们进入下一个环节：信号到达交换机后，交换机是如何工作的。与集线器简单地广播信号不同，交换机能够"认出"每个包的目的地，将包精准地送到对应的端口。
...
![](./images/131213335.png)

自动协商功能就利用了这样的脉冲信号，即通过这种信号将自己能够支持的工作模式和传输速率相互告知对方，并从中选择一个最优的组合

交换机的工作范围仅限于同一个以太网内，它只能根据 MAC 地址在局域网内部转发包。当包的目的地不在本地网络时，就需要路由器出场了。路由器基于 IP 地址工作，能够将包跨越不同的网络转发到更远的地方。下面我们来看路由器的包转发操作。

## 路由器的包转发操作

路由器和交换机一样也负责对包进行转发，但它们的工作方式有一些差异。交换机是基于以太网规格工作的设备，而路由器是基于 IP 工作的

### 路由器的基本知识

![](./images/131213701.png)
...
以上就是路由器进行包转发的核心流程。但在实际应用中，特别是接入互联网的路由器，通常还承担着地址转换和包过滤等重要功能。这些附加功能对于理解真实的网络通信至关重要，我们接下来逐一讲解。
...
本文沿着网络包从网卡出发后的传输路径，依次讲解了四个关键环节：

1. **信号在网线和集线器中传输**：信号以正负电压的形式在双绞线中传播，双绞线的"绞"用来抑制噪声，集线器则将信号原样广播到所有端口
2. **交换机的包转发操作**：交换机将信号还原为数字信息，根据 MAC 地址表将包精准转发到目标端口，同时维护地址表并支持全双工通信
3. **路由器的包转发操作**：路由器基于 IP 地址查询路由表确定转发目标，丢弃 MAC 头部后重新封装，通过 TTL 控制包的有效期，并支持分片功能
4. **路由器的附加功能**：地址转换（NAT）通过改写 IP 地址和端口号实现私有网络与互联网的通信，包过滤则根据规则对包进行放行或丢弃

从物理信号到数字包，从局域网到跨网络转发，这一路走来，网络包完成了从"出门"到"上路"的全过程。在下一篇中，我们将进入接入网和运营商的领域，看看包如何跨越互联网到达最终目的地。
