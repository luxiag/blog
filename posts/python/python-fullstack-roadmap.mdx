...
<ProgressIndicator
  ...
  items={[
    ...
    { slug: "python/fastapi-from-zero-roadmap", title: "FastAPI 零基础路线" },
...
零基础学习者应先进入 [FastAPI 零基础学习路线](/posts/python/fastapi-from-zero-roadmap)，按路线顺序逐步完成前五篇课程，不要跳过前置练习；完成后再用 [FastAPI 综合复习与工程索引](/posts/python/fastapi-complete-guide) 串联 Pydantic v2 输入输出模型、依赖注入、异常映射、生命周期与测试客户端，并从索引继续进入后续进阶专题。前端经验能帮助你设计消费体验，但**不要让页面组件结构决定数据库表结构**。
...
## 9. 阶段 6：FastAPI 部署观测与 Linux 生产闭环

**目标**：从“本机能跑”升级为可安全发布、观测、回滚和恢复。

**前置**：阶段 5；应用已经通过真实依赖的集成测试，并具备迁移命令。

先完成 [FastAPI 生产可观测性与部署](/posts/python/fastapi-observability-deployment)，为应用加入稳定错误契约、Request ID、结构化日志、liveness/readiness、不可变镜像、迁移编排、优雅停止、冒烟验证与回滚步骤。确认 FastAPI 服务本身形成可验证的部署闭环后，再通过 [Nginx 完全指南](/posts/nginx/nginx-complete-guide) 理解反向代理、TLS、超时、请求体限制和真实客户端地址，并沿 [Linux 生产工程路线图](/posts/linux/linux-production-roadmap) 补齐主机基础、可靠 CI/CD、数据库运维、备份恢复、安全与事件响应；需要进一步建设平台级 SLI/SLO 时，再进入 [可观测性工程：Metrics、Logs、Traces 与 SLI/SLO 告警](/posts/linux/observability-sli-slo-alerting)。
