...
5. [Part 5 · PostgreSQL 深度实战](/posts/postgresql/postgresql-guide) —— 把进阶知识落到一个具体数据库
:::
...
我们会刻意保持**数据库中立**——讲的是所有关系型数据库（MySQL、PostgreSQL、SQL Server、Oracle、SQLite）共通的原理。具体到某个数据库的命令细节会标注差异，但思维方式是通用的。读完这一篇，你再去看 [Part 5 的 PostgreSQL 专精](/posts/postgresql/postgresql-guide)，会发现那些"高级特性"背后都是这里讲的同一套逻辑。
...
事务保证一组操作"要么全成、要么全败"，由 ACID 描述：原子性、一致性、隔离性、持久性（详见 [Part 1 第 12 节](/posts/sql/sql-beginner-guide)）。
...
代价是旧版本数据需要后台清理（PG 的 `VACUUM`、InnoDB 的 purge），否则会膨胀——这部分在 [Part 5](/posts/postgresql/postgresql-guide) 会展开。
...
**下一步：** 进入系列最后一篇 [Part 5 · PostgreSQL 深度实战](/posts/postgresql/postgresql-guide)，看这些通用原理如何在一个功能最完善的开源数据库上具体落地：PG 的 MVCC 实现、丰富的索引类型（GIN/GiST/BRIN）、`EXPLAIN` 实战、`pg_stat_statements` 找 Top SQL、`VACUUM` 与表膨胀、连接池、分区表，以及一个完整的博客系统数据模型设计。
