...
- **packet loss**：丢包率。应与同地域、同线路、同时间段的历史结果对比；少量探测中的 0% 或某个固定百分比都不能单独证明链路可靠或异常。
- **time**：往返延迟（RTT）。距离、运营商路由、云网络和业务时段都会影响结果，应先建立自己的正常基线，再观察突增和持续偏离。
- **ttl**：每经过一个路由器减 1，可粗略估算经过的跳数。

:::warning 阈值只能作为基线，不能作为绝对结论
“丢包超过某个百分比”或“延迟超过某个毫秒数”都不是适用于所有网络的故障线。短时间样本、ICMP 限速、跨境链路和移动网络都可能让结果波动。判断异常时要结合历史基线、连续观测、TCP/UDP 端口测试和应用层指标。
:::
...
:::note ping 不通 ≠ 服务不可用
很多云服务器的防火墙/安全组会**屏蔽或限速 ICMP**（ping 使用的协议），所以 ping 不通不代表服务挂了，也不能成为停止排查的条件。无论 ping 是否有响应，都应继续针对业务实际使用的协议和端口测试：

```bash
# TCP 端口，例如 SSH
nc -zv -w 5 example.com 22

# UDP 端口，例如 DNS；UDP 无握手，结果需结合服务端抓包或协议客户端判断
nc -zvu -w 5 8.8.8.8 53

# HTTP/HTTPS 应用层
curl -v --connect-timeout 5 https://example.com/health
```

反过来，ping 通了也只能说明 IP 层有 ICMP 响应，不代表 TCP、UDP 或具体应用正常。
:::
...
```mermaid
graph TD
  START["网络异常"] --> DNS{"DNS 解析正常？<br/>dig +short domain"}
  DNS -- "失败" --> DNS_FIX["检查 /etc/hosts<br/>换 DNS 服务器<br/>检查域名记录"]
  DNS -- "正常" --> PING{"ICMP 探测有响应？<br/>ping IP"}
  PING -- "无响应" --> ROUTE["记录 ICMP 可能被屏蔽<br/>traceroute 辅助定位路由"]
  PING -- "有响应" --> PORT["继续测试目标端口<br/>TCP: nc/curl<br/>UDP: nc -u 或协议客户端"]
  ROUTE --> PORT
  PORT -- "连接失败" --> LISTEN["服务端检查监听<br/>ss -lntup"]
  LISTEN --> FIREWALL{"防火墙和安全组放行？<br/>ufw status / iptables -L"}
  FIREWALL -- "未放行" --> OPEN["放行所需 TCP/UDP 端口<br/>同步检查云安全组"]
  FIREWALL -- "已放行" --> SERVICE["检查服务状态和日志"]
  PORT -- "端口可达" --> APP{"curl/协议客户端测试<br/>应用响应正常？"}
  APP -- "异常" --> APP_DEBUG["检查应用日志<br/>检查 upstream 配置"]
  APP -- "正常" --> OK["继续检查客户端、代理和业务逻辑"]
```
...
如果磁盘是全新的（没有分区表），需要先创建分区表和分区。大于 2 TiB 的磁盘应使用 GPT；**现代 util-linux 提供的 `fdisk` 已经支持 GPT**，因此并不存在“GPT 必须用 parted、fdisk 只能管理 MBR”的限制。`fdisk` 与 `parted` 都可以使用，选择团队熟悉且发行版已验证的工具即可。

**使用现代 fdisk 创建 GPT 分区**：
...
```
Command (m for help): g       # 创建新的 GPT 分区表（会清除原分区信息）
Command (m for help): n       # 新建分区
Partition number: 1           # 分区号
First sector: 回车            # 默认起始位置
Last sector: 回车             # 默认使用全部空间
Command (m for help): p       # 打印结果，写入前再次确认
Command (m for help): w       # 写入分区表
```

如果磁盘因兼容旧系统需要 MBR，可在 `fdisk` 中用 `o` 创建 DOS/MBR 分区表；新服务器通常优先 GPT。
...
传统分区并非一旦创建就完全不能扩。如果云平台已经把底层磁盘扩容，而且目标分区后方存在**足够、连续、相邻的未分配空间**，通常可以先扩大分区边界，再扩大其中的文件系统。操作前务必确认设备名、文件系统类型并做好可恢复备份：
...
以 `/dev/vdb1` 为例，可以优先使用云服务器常见的 `growpart`：

```bash
# Ubuntu/Debian：growpart 由 cloud-guest-utils 提供
sudo apt install cloud-guest-utils -y

# 扩大 /dev/vdb 的第 1 个分区，使用其后全部可用空间
sudo growpart /dev/vdb 1
```

也可以使用现代 `parted` 调整分区终点：

```bash
# 将第 1 个分区扩到磁盘末尾；执行前再次核对分区号
sudo parted /dev/vdb --script 'resizepart 1 100%'

# 请求内核重新读取分区表；若设备繁忙且读取失败，按维护窗口安排重启
sudo partprobe /dev/vdb
```

分区变大后，文件系统不会自动占满新空间，还要按类型扩容：

```bash
# ext4：参数是块设备；常见场景支持挂载状态下扩容
sudo resize2fs /dev/vdb1

# XFS：参数是挂载点，扩容时文件系统必须已挂载
...
:::warning 传统分区扩容有明确前提
如果未分配空间不与目标分区尾部相邻、中间还有其他分区，或底层存储尚未扩容，就不能直接照抄上面的命令。分区表重读、文件系统在线扩容和业务不停机能力还取决于内核、设备映射、文件系统与业务写入边界；重要数据应先备份并在维护窗口中操作。
:::
...
传统分区在尾部有相邻未分配空间时也能扩容，但遇到跨磁盘聚合、空间重新分配或复杂布局时不够灵活。**LVM（Logical Volume Manager）** 在物理磁盘和文件系统之间增加一层抽象，让你可以按卷组统一管理容量，并在满足底层存储和文件系统条件时动态扩展逻辑卷。
...
- **LV（Logical Volume，逻辑卷）**：从 VG 中划分出来的逻辑分区，在上面建文件系统然后挂载。LV 扩展通常较灵活；缩小则必须同时满足文件系统支持和安全操作流程，XFS 明确不支持缩小。
...
### 10.4 扩容逻辑卷：先确认在线操作边界

LVM 可以在很多场景中扩展已挂载的逻辑卷，但“在线扩容”不是 LVM 单方面保证的能力。是否需要维护窗口，取决于云盘或阵列能否在线扩展、内核能否识别新容量、设备映射链路、文件系统是否支持在线 grow，以及数据库等业务是否允许在变更期间继续写入。生产操作前应核对备份、监控、回滚方案和厂商文档。

下面以底层容量已就绪、VG 有足够空闲空间，且 ext4/XFS 与业务都允许在线扩展为前提：/data 用了 50G 快满了，要把 lv_data 从 50G 扩到 100G。
...
这就是 LVM 的弹性：**空间不够时，可以按“底层存储 → PV/VG → LV → 文件系统”逐层扩展**。其中哪些步骤能够在线执行，要以实际存储栈、文件系统能力和业务操作边界为准；不能把“使用 LVM”等同于“任何环境都能全程不停机”。

::: details LVM 缩容（了解即可）
缩小逻辑卷的风险远高于扩容，必须先确认文件系统本身支持 shrink，再按其要求卸载、检查并先缩文件系统，最后才能缩 LV。**XFS 不支持缩小文件系统**，因此不能通过 `lvreduce` 安全地缩小其中的 XFS；强行先缩 LV 会直接截断数据。实际环境通常新建更小的 LV 和文件系统，迁移并校验数据后再切换。ext4 虽支持离线缩小，也应先备份并安排维护窗口。
:::
...
**swappiness** 是影响内核回收匿名页与文件页取舍的参数之一。它不是“内存用到多少百分比才开始 swap”的开关，**可接受取值、默认值和具体语义都应以当前内核与发行版文档为准**；容器宿主机、数据库、桌面和不同存储介质也没有一个通用最佳值。
...
如果监控显示持续换页正在影响延迟，或工作负载文档明确要求调整，应先在测试环境和业务低峰做临时修改。下面不提供通用固定数字，`TARGET_SWAPPINESS` 必须来自当前内核文档、负载测试与监控结果：

```bash
# 先显式提供经过验证的目标值，例如由变更系统注入
: "${TARGET_SWAPPINESS:?请先根据当前系统和压测结果设置 TARGET_SWAPPINESS}"
...
printf 'vm.swappiness=%s\n' "$TARGET_SWAPPINESS" | \
  sudo tee /etc/sysctl.d/90-myapp-swappiness.conf
sudo sysctl --system
```

:::tip 用观测结果而不是通用推荐值决策
调低不一定更快：过度回避 swap 可能压缩文件缓存并更早触发 OOM；调高也不一定更稳，慢存储上的频繁换页会放大尾延迟。调整时要结合内核版本、发行版配置、RAM 与存储速度、应用内存模型、`vmstat`/PSI 指标和真实业务延迟，并记录可回退的原值。
:::
...
| 问题层级 | 工具 | 解决什么问题 |
| :--- | :--- | :--- |
| L3 网络层 | `ping`、`traceroute` | ICMP 是否有响应、路由路径是否异常（仅作为旁证） |
| L4 传输层 | `ss`、`nc`、协议客户端 | TCP/UDP 端口是否可达，谁在监听 |
| L7 应用层 | `curl`、`dig` | HTTP 是否正常，DNS 是否正确 |
| 防火墙 | `ufw`、`iptables` | 规则是否放行，哪层拦截了 |

核心方法论：**DNS → ICMP/路由旁证 → TCP/UDP 端口 → 防火墙/安全组 → 应用协议**，从底层到高层、从外到内逐层排查。`ping` 无响应时不能终止流程，仍要继续测试业务实际使用的端口和 HTTP 等应用协议。
...
| 扩展传统分区 | 尾部相邻空闲空间 → `growpart`/`parted` → `resize2fs`/`xfs_growfs` |
| 弹性扩容 | LVM（PV → VG → LV → 文件系统；在线能力按实际环境确认） |
| 内存安全网 | Swap 文件（创建 → swapon → 按内核、负载与观测调整 swappiness） |
