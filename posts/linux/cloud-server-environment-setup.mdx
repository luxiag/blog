...
### 3.3 Redis 持久化：别让缓存数据丢了

Redis 是内存数据库，默认数据只存在内存中，重启就没了。生产环境必须配置持久化，否则一次意外重启就能让你丢失所有缓存数据。

Redis 提供两种持久化方式：

| 方式 | 原理 | 优点 | 缺点 |
| :--- | :--- | :--- | :--- |
| **RDB** | 定时把内存快照写入磁盘 | 恢复快，文件紧凑 | 可能丢失最近几分钟的数据 |
| **AOF** | 把每条写命令追加到日志文件 | 最多丢 1 秒数据 | 文件更大，恢复较慢 |

Ubuntu 默认开启了 RDB。建议同时开启 AOF 以获得更好的数据安全：
...
Nginx 的匹配规则是：请求到达后，优先匹配 `server_name` 与请求头中 `Host` 字段一致的 `server` 块。如果都不匹配，走默认站点（`sites-enabled` 中按字母序第一个，或显式标记 `default_server` 的）。
...
默认的访问日志格式包含：客户端 IP、请求方法与路径、状态码、响应大小、耗时等。结合第一篇学的 `awk`，可以快速分析：

```bash
# 统计每个状态码出现的次数
awk '{print $9}' /var/log/nginx/access.log | sort | uniq -c | sort -nr

# 找出耗时最长的 10 个请求（默认格式最后一列是耗时）
...
- **数据库**：安装 MySQL 与 Redis，遵循**最小权限**和**绝不暴露公网**两条铁律；MySQL 备份与 utf8mb4 字符集、Redis AOF 持久化都不容忽视。
- **Nginx**：作为流量入口，理解并配置**反向代理**，托管静态资源，多站点配置，常见错误排错；再用 **Certbot** 一键上 HTTPS。
- **systemd**：把 Nginx、数据库乃至**你自己的应用**都管理成开机自启、崩溃自愈的可靠服务，并用 `journalctl` 查日志。

至此，环境万事俱备。下一篇《[系统运维与安全](/posts/linux/system-ops-and-security)》，我们将为这台跑着服务的服务器装上"仪表盘"和"安保系统"——日志查询、定时任务、性能监控与安全加固，让它从"能跑"变成"能放心跑"。
