...
相对路径图片自动解析（文件放在 `posts/<category>/images/` 下）：

```md
![替代文本](./images/screenshot.png)
```

点击图片会打开 Lightbox 全屏预览。

### 链接

内部链接不带 `/blog` 前缀（basePath 已在 next.config.ts 配置）：

```md
[链接文本](/posts/dotnet/csharp-basic-syntax)
```

外部链接自动在新标签页打开，并显示外链图标。

---

## MDX 语法

`.mdx` 文件在 Markdown 基础上支持 JSX 表达式和组件：

### 变量与表达式

花括号内可以写 JavaScript 表达式，结果会被计算并渲染到页面上。例如 `2 + 2 的结果是 {2 + 2}` 会渲染为 "2 + 2 的结果是 4"。

### 条件渲染

使用逻辑与运算符 `&&` 实现条件渲染：`{someCondition && <Component />}`，当 `someCondition` 为真时才渲染后面的内容。

### 组件属性传值

通过属性向组件传递数据，如 `items` 数组、`title` 字符串等。属性用花括号 `{}` 包裹表示 JavaScript 表达式，用引号 `""` 包裹表示字符串字面量。

### 多行属性

属性较多时可以换行书写，每行一个属性，最后用 `/>` 自闭合。

### 内联样式

`style` 属性接受对象语法，外层花括号 `{}` 是 JSX 表达式，内层花括号 `{}` 是样式对象，如 `style={{ color: "red" }}`。

### HTML 标签

MDX 中可以直接使用 HTML 标签，但推荐使用 `::: details` 语法代替原生 `<details>`，会自动获得增强样式和动画效果。

### 注释

JSX 风格注释写在花括号内的 `/* */` 中，如 `{/* 注释内容 */}`，不会渲染到页面上。
```
```
