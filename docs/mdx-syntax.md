...
也支持自定义标题，标题中的行内 Markdown（如强调和行内代码）会保留对应样式。标题里出现 Shell 变量或其他以 `$` 开头的非数学内容时，应使用行内代码（如 `` `$PATH` ``、`` `$@` ``）或转义 `$`，避免被 `remark-math` 与后文的 `$` 跨行识别为公式：
...
组件会把第一个直接标题的层级作为步骤层级（推荐统一使用 `###`），之后只有同级标题会开始新步骤；更深层标题仍属于当前步骤。标题会保留真实的 heading 元素、`id` 和行内格式，因此文章 TOC 可以正常定位。

标题前的内容会作为步骤说明显示，不会被丢弃；完全没有标题时组件会原样渲染内容，并在开发环境给出中文警告。步骤正文可以包含代码块、列表、告示块、Details、CodeTabs 等内容。编号由组件生成，标题通常不需要再手写“步骤 1”。
...
### 14. 系列目录（ProgressIndicator）

用于系列文章顶部，展示当前阅读位置、系列全貌以及上一篇/下一篇导航：
...
- `current` 是当前文章 slug，必须与 `items` 中的一项一致。
- `items` 按系列阅读顺序排列；当前篇之前的条目显示为“已完成”，这是顺序推断，不代表真实浏览历史。
- 顶部直接显示醒目的 `当前序号 / 总篇数`、当前文章标题和分段式阅读路线；每个分段对应一篇文章，绿色表示已完成、蓝色表示当前篇、灰色表示后续文章。
- 7 篇以内默认展开；长系列默认折叠，点击“查看 N 篇目录”后以单列/双列滚动目录展示所有文章。
- 当前篇不可重复跳转，其他已完成或未读文章均可点击；底部导航始终相对真实当前篇。
- 链接默认生成为 `/posts/<slug>`。如传入 `basePath`，组件会规范化首尾斜杠；文章内不要添加 `/blog` 前缀。
- 空列表、当前 slug 未收录、空字段和重复 slug 会在开发环境给出中文警告。
